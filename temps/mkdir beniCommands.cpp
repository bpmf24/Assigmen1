...
bool BaseCommand::isFileExists(string args, FileSystem &fs) {
    ...
    for (size_t j = 0; j < fileNames.size(); j++) {
        ...
        if (fileNames[j] == "..") {
            ...
            if (start->getChildren().size() == 0 && j < fileNames.size()) {
                return false;
            } else {
                //go over all (start->getChildren()), check if directory and name = fileName
                for (int i = 0; i < (start->getChildren().size()); i++) {
                    if (j == fileNames.size() - 1) {
                        if (typeid(*(start->getChildren()[i])) == typeid(File) &&
                            (start->getChildren()[i]->getName() == fileNames[j])) {
                            return true;
                        } else if (typeid(*(start->getChildren()[i])) == typeid(Directory) &&
                                   (start->getChildren()[i]->getName() == fileNames[j])) {
                            return true;
                        }
                    }
                    if ((typeid(*(start->getChildren())[i])) == typeid(Directory) && (start->getChildren()[i]->getName() == fileNames[j])) {
                        start = dynamic_cast<Directory *>(start->getChildren()[i]);
                        break;
                    }
                    if (i >= (start->getChildren()).size()-1) { //gone through all (start->getChildren()) and didnt find next directory
                        return false;
                    }
                }
            }
